...
class RFDETRPoseLargeConfig(RFDETRPoseConfig):
    """Configuration for RF-DETR Pose Large.

    The previous defaults attempted to load `rf-detr-large.pth` while using the
    Medium-style pose architecture (hidden_dim=256, patch_size=16, etc.), which
    causes a shape-mismatch failure during `load_state_dict()`.

    This config is aligned with the RF-DETR Large detection checkpoint so that
    the weights can be loaded successfully.
    """

    # Match RF-DETR Large detection architecture
    encoder: Literal["dinov2_windowed_small", "dinov2_windowed_base"] = "dinov2_windowed_base"
    hidden_dim: int = 384
    sa_nheads: int = 12
    ca_nheads: int = 24
    ...
    # Keep the default resolution consistent with the checkpoint family.
    # (Note: inputs must be divisible by patch_size * num_windows = 56.)
    resolution: int = 560

    # Decoder depth can be increased for pose fine-tuning, but the pretrain
    # checkpoint corresponds to the 3-layer detector.
    dec_layers: int = 3
